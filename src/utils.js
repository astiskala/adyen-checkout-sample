...
const getPaymentsDefaultConfig = async () => {
  ...
  config.billingAddress.country = await httpGet(
    "env",
  ...
  config.deliveryAddress.country = await httpGet(
    "env",
  ...
  config.paymentMethod.clickAndCollect = await httpGet(
    "env",
    "ZIP_CLICKANDCOLLECT"
  );
...
const makePayment = (
  ...
  return getPaymentsDefaultConfig().then((paymentsDefaultConfig) => {
    ...
    if (paymentMethod.paymentMethod.type === "zip") {
      paymentMethod.paymentMethod.clickAndCollect =
        paymentsConfig.paymentMethod.clickAndCollect;
    }
